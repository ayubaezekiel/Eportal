...
function HODDashboard() {
  ...
  const departmentCourses =
    courses?.filter((c) => c.departmentId === session?.user.departmentId) || [];
  const departmentStaff =
    users?.filter((u) => u.departmentId === session?.user.departmentId) || [];
  const pendingResults =
    results?.filter((r) => r.results.status === "Pending") || [];
